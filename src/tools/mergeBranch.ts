...
export function registerMergeBranchTool(server: McpServer) {
    server.registerTool(
        "merge_branch",
        {
            title: "Fusionar rama",
            ...
            inputSchema: {
                owner: ownerSchema,
                repo: repoNameSchema,
                head: requiredBranchSchema.describe(
                    "Rama de origen que se quiere fusionar (ej. 'feature/mi-cambio').",
                ),
        ...
        async ({ owner, repo, head, base, commit_message }) => {
            try {
                ...
                if (data === null) {
                    return ok(
                        `La rama '${head}' ya estaba al día con '${base}'. No se creó ningún commit.`,
                    );
                }

                return ok(
                    `Fusión completada: '${head}' → '${base}'\n` +
